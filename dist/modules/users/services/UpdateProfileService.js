...
let UpdateProfile = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const userWithUpdateProfile = await this.usersRepository.findByEmail(email);

    if (userWithUpdateProfile && userWithUpdateProfile.id !== user_id) {
    ...
    if (password && !old_password) {
      throw new _AppError.default('You need to inform the old password');
    }
    ...
    if (password && old_password) {
      ...
      if (!checkOldPassword) {
        throw new _AppError.default('Wrong password');
      }
